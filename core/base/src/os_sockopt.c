...
int os_nonblocking(os_socket_t fd)
{
    ...
    if (rc != OS_OK) {
        os_logsp(ERROR, ERRNOID, os_socket_errno, "ioctlsocket failed");
        return OS_ERROR;
    }
#else
    int rc;
    ...
    if (flags < 0) {
        os_logsp(ERROR, ERRNOID, os_socket_errno, "F_GETFL failed");
        return OS_ERROR;
    }
    if (!(flags & O_NONBLOCK)) {
        rc = fcntl(fd, F_SETFL, (flags | O_NONBLOCK));
        if (rc != OS_OK) {
            os_logsp(ERROR, ERRNOID, os_socket_errno, "F_SETFL failed");
            return OS_ERROR;
        }
    }
#endif
...
int os_closeonexec(os_socket_t fd)
{
    ...
    if (flags < 0) {
        os_logsp(ERROR, ERRNOID, os_socket_errno, "F_GETFD failed");
        return OS_ERROR;
    }
    if (!(flags & FD_CLOEXEC)) {
        rc = fcntl(fd, F_SETFD, (flags | FD_CLOEXEC));
        if (rc != OS_OK) {
            os_logsp(ERROR, ERRNOID, os_socket_errno, "F_SETFD failed");
            return OS_ERROR;
        }
    }
#endif
...
int os_listen_reusable(os_socket_t fd, int on)
{
    ...
    os_log(DEBUG, "Turn on SO_REUSEADDR");
    rc = setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, (void *)&on, sizeof(int));
    if (rc != OS_OK) {
        os_logsp(ERROR, ERRNOID, os_socket_errno, "setsockopt(SOL_SOCKET, SO_REUSEADDR) failed");
        return OS_ERROR;
    }
#endif
...
int os_tcp_nodelay(os_socket_t fd, int on)
{
   ...
   os_log(DEBUG, "Turn on TCP_NODELAY");
    rc = setsockopt(fd, IPPROTO_TCP, TCP_NODELAY, (void *)&on, sizeof(int));
    if (rc != OS_OK) {
        os_logsp(ERROR, ERRNOID, os_socket_errno, "setsockopt(IPPROTO_TCP, TCP_NODELAY) failed");
        return OS_ERROR;
    }
#endif
...
int os_so_linger(os_socket_t fd, int l_linger)
{
    ...
    os_log(DEBUG, "SO_LINGER:[%d]", l_linger);
    rc = setsockopt(fd, SOL_SOCKET, SO_LINGER,
            (void *)&l, sizeof(struct linger));
    if (rc != OS_OK) {
        os_logsp(ERROR, ERRNOID, os_socket_errno, "setsockopt(SOL_SOCKET, SO_LINGER) failed");
        return OS_ERROR;
    }
#endif
...
int os_bind_to_device(os_socket_t fd, const char *device)
{
    ...
    os_log(DEBUG, "SO_BINDTODEVICE:[%s]", device);
    rc = setsockopt(fd, SOL_SOCKET, SO_BINDTODEVICE, device, strlen(device)+1);
    if (rc != OS_OK) {
        int err = os_errno;
         os_logsp(ERROR, ERRNOID, os_socket_errno,"setsockopt(SOL_SOCKET, SO_BINDTODEVICE) failed");
        if (err == OS_EPERM)
            os_log(ERROR, "You need to grant CAP_NET_RAW privileges to use SO_BINDTODEVICE");
        return OS_ERROR;
    }
#endif
