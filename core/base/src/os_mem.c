...
#undef OS_LOG_DOMAIN
#define OS_LOG_DOMAIN __os_mem_domain
...
void *os_malloc_debug(size_t size, const char *file_line)
{
    ...
    if (!buf) {
        OS_ERR("os_buf_alloc_debug[headroom:%d, size:%d] failed",
...
void *os_calloc_debug(size_t nmemb, size_t size, const char *file_line)
{
    ...
    if (!ptr) {
        OS_ERR("os_malloc_debug[nmemb:%d, size:%d] failed",
                (int)nmemb, (int)size);
        return NULL;
    }
...
void *os_realloc_debug(void *ptr, size_t size, const char *file_line)
{
    ...
    if (!buf) {
        OS_ERR("Cannot get buf from ptr[%p], headroom[%d]",
                ptr, (int)headroom);
        return NULL;
    }
    ...
    if (size > (cluster->size - headroom)) {
        ...
        if (!new) {
            OS_ERR("os_malloc_debug[%d] failed", (int)size);
            return NULL;
        }
