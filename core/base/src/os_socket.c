...
os_sock_t *os_sock_socket(int family, int type, int protocol)
{
    ...
    OS_DEBUG("socket create(%d:%d:%d)", sock->family, type, protocol);
...
int os_sock_bind(os_sock_t *sock, os_sockaddr_t *addr)
{
    ...
    if (bind(sock->fd, &addr->sa, addrlen) != 0) {
        OS_LOG_MESSAGE(OS_TLOG_ERROR, os_socket_errno,
                ...
                addr->os_sa_family, OS_ADDR(addr, buf), OS_PORT(addr));
        return OS_ERROR;
    }

    memcpy(&sock->local_addr, addr, sizeof(sock->local_addr));

    OS_DEBUG("socket bind %s:%d", OS_ADDR(addr, buf), OS_PORT(addr));
...
int os_sock_listen(os_sock_t *sock)
{
    ...
    rc = listen(sock->fd, 5);
    if (rc < 0) {
        OS_LOG_MESSAGE(OS_TLOG_ERROR, os_socket_errno, "listen failed");
        return OS_ERROR;
    }
...
#endif
    if (r != 0) {
        OS_LOG_MESSAGE(OS_TLOG_ERROR, os_socket_errno, "closesocket failed");
        return OS_ERROR;
    }
