...
#if 0
int os_getnameinfo(
    char *hostname, socklen_t hostname_len, os_sockaddr_t *addr, int flags)
{
    os_assert(hostname);
    os_assert(addr);

    return getnameinfo(&addr->sa, os_sockaddr_len(addr),
...
{
    *sa_list = NULL;
    return os_addaddrinfo(sa_list, family, hostname, port, flags);
}

int os_freeaddrinfo(os_sockaddr_t *sa_list)
{
    os_sockaddr_t *next = NULL, *addr = NULL;

    addr = sa_list;
    while (addr) {
        next = addr->next;
        if (addr->hostname)
            os_free(addr->hostname);
        os_free(addr);
        addr = next;
    }
        ...
        int family, const char *hostname, uint16_t port, int flags)
{
    ...
    os_snprintf(service, sizeof(service), "%u", port);

    rc = getaddrinfo(hostname, service, &hints, &ai_list);
    if (rc != 0) {
        os_logsp(ERROR, ERRNOID, os_socket_errno, "getaddrinfo(%d:%s:%d:0x%x) failed", family, hostname, port, flags);
        return OS_ERROR;
    }
...
{
    os_sockaddr_t *addr = NULL, *prev = NULL, *next = NULL;
...
int os_copyaddrinfo(os_sockaddr_t **dst, const os_sockaddr_t *src)
{
    os_sockaddr_t *d;
    const os_sockaddr_t *s;

    for (*dst = d = NULL, s = src; s; s = s->next) {
        if (!d) {
            *dst = d = os_memdup(s, sizeof *s);
            if (!(*dst)) {
                os_log(ERROR, "os_memdup() failed");
                return OS_ERROR;
            }
        } else {
...
{
    os_sockaddr_t *head = NULL, *addr = NULL, *new = NULL, *old = NULL;
    ...
    while (old) {
        ...
        if (head == NULL || addr->os_sa_family == family) {
            addr->next = head;
            head = addr;
        } else {
            new = head;
            while(new->next != NULL && new->next->os_sa_family != family) {
                new = new->next;
            }
            addr->next = new->next;
            new->next = addr;
...
os_sockaddr_t *os_link_local_addr(const char *dev, const os_sockaddr_t *sa)
{
    ...
    for (cur = iflist; cur != NULL; cur = cur->ifa_next) {
        os_sockaddr_t *ifa_addr = NULL;
        os_sockaddr_t *addr = NULL;

        ifa_addr = (os_sockaddr_t *)cur->ifa_addr;

        if (ifa_addr == NULL) /* may happen with ppp interfaces */
            continue;

        if (ifa_addr->os_sa_family == AF_INET)
            continue;
...
{
    os_assert(dev);
    return os_link_local_addr(dev, NULL);
}

os_sockaddr_t *os_link_local_addr_by_sa(const os_sockaddr_t *sa)
{
    os_assert(sa);
    return os_link_local_addr(NULL, sa);
}
        ...
        int no_ipv4, int no_ipv6, int prefer_ipv4)
{
    ...
    if (no_ipv4 == 1) {
        rv = os_filteraddrinfo(addr, AF_INET6);
        os_assert(rv == OS_OK);
    }
    if (no_ipv6 == 1) {
        rv = os_filteraddrinfo(addr, AF_INET);
        os_assert(rv == OS_OK);
    }

    if (prefer_ipv4 == 1) {
        rv = os_sortaddrinfo(addr, AF_INET);
        os_assert(rv == OS_OK);
    } else {
        rv = os_sortaddrinfo(addr, AF_INET6);
        os_assert(rv == OS_OK);
    }
...
{
    int family;
    os_sockaddr_t *sockaddr = NULL;

    sockaddr = sa;
    os_assert(sockaddr);
    os_assert(buf);
    os_assert(buflen >= OS_ADDRSTRLEN);

    family = sockaddr->os_sa_family;
    switch (family) {
    case AF_INET:
    ...
    case AF_INET6:
        return inet_ntop(family, &sockaddr->sin6.sin6_addr, buf,
                INET6_ADDRSTRLEN);
    default:
        os_log(FATAL, "Unknown family(%d)", family);
        os_abort();
        return NULL;
    }
}
...
{
    os_sockaddr_t *dst = NULL;

    os_assert(src);
    dst = sa;
    os_assert(dst);

    dst->os_sa_family = family;
    switch(family) {
    case AF_INET:
...
{
    const os_sockaddr_t *sockaddr = sa;

    os_assert(sa);

    switch(sockaddr->os_sa_family) {
    case AF_INET:
        return sizeof(struct sockaddr_in);
    case AF_INET6:
        return sizeof(struct sockaddr_in6);
    default:
        os_log(FATAL, "Unknown family(%d)", sockaddr->os_sa_family);
        os_abort();
        return OS_ERROR;
    }
}
...
bool os_sockaddr_is_equal(const void *p, const void *q)
{
    ...
    if (a->os_sa_family != b->os_sa_family)
        return false;

    switch (a->os_sa_family) {
    case AF_INET:
    ...
    case AF_INET6:
        ...
        return true;
    default:
        os_log(ERROR, "Unexpected address faimily %u", a->os_sa_family);
        os_abort();
    }
}
        ...
        os_ipsubnet_t *ipsub, const char *ipstr, int network_allowed)
{
    ...
    if (rc == 1) {
        if (IN6_IS_ADDR_V4MAPPED((struct in6_addr *)ipsub->sub)) {
             ...
             */
            os_log(ERROR, "Cannot support IPv4-mapped IPv6: "
        ...
        const char *ipstr, const char *mask_or_numbits)
{
    ...
    if (!looks_like_ip(ipstr)) {
        os_log(ERROR, "looks_like_ip(%s, %s) failed", ipstr, mask_or_numbits);
        return OS_ERROR;
    }
    ...
    if (mask_or_numbits) {
        ...
        if (*endptr == '\0' && bits > 0 && bits <= maxbits) {
            ...
            os_log(ERROR, "Bad netmask %s", mask_or_numbits);
            return OS_ERROR;
        }
    }
